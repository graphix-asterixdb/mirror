...
public class MetadataBootstrap {
    ...
    public static void startUniverse(INCServiceContext ncServiceContext, boolean isNewUniverse)
        ...
        try {
            for (int i = 0; i < PRIMARY_INDEXES.length; i++) {
                enlistMetadataDataset(ncServiceContext, PRIMARY_INDEXES[i], MetadataBootstrap::isCatalogUpgradeLegal);
            }
    ...
    public static void enlistMetadataDataset(INCServiceContext ncServiceCtx, IMetadataIndex index,
            Function<IMetadataIndex, Boolean> catalogUpgradabilityChecker)
        ...
        if (isNewUniverse()) {
            ...
            if (createMetadataDataset) {
                if (!catalogUpgradabilityChecker.apply(index)) {
                    throw new AsterixException(ErrorCode.CANNOT_UPGRADE_CATALOG, index.getIndexName());
                }
            }
        }
        if (createMetadataDataset) {
