...
/**
 * Create an initial path, which consists of zero hops and a single vertex. We expect the following as arguments:
 * <ol>
 *   <li>A vertex field, which will act as the source vertex.</li>
 * </ol>
 */
public class CreateNewZeroHopDescriptor extends AbstractScalarFunctionDynamicDescriptor {
            ...
            public IScalarEvaluator createScalarEvaluator(IEvaluatorContext ctx) throws HyracksDataException {
                return new IScalarEvaluator() {
                    ...
                    public void evaluate(IFrameTupleReference tuple, IPointable result) throws HyracksDataException {
                        // If our source is NULL or MISSING, then our output path is NULL or MISSING.
                        sourceArgEval.evaluate(tuple, sourceArgPtr);
                        if (PointableHelper.checkAndSetMissingOrNull(result, sourceArgPtr)) {
                            return;
                        }
                        ...
                        try {
                            ...
                            int startOfEdgeList = pathHeaderSize + sourceItemSize;
                            dataOutput.writeInt(startOfEdgeList);

                            // Write our edge list size (which is zero, so we'll write the same value again).
                            dataOutput.writeInt(startOfEdgeList);

                            // Write the number of edges (0).
                            dataOutput.writeInt(0);

                            // Write our source vertex.
    ...
    public FunctionIdentifier getIdentifier() {
        return GraphixFunctionIdentifiers.CREATE_NEW_ZERO_HOP_PATH;
    }
}
