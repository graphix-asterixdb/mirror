...
public class OperatorDeepCopyVisitor implements ILogicalOperatorVisitor<ILogicalOperator, Void> {
    ...
    @Override
    public ILogicalOperator visitMarkerSinkOperator(MarkerSinkOperator op, Void arg) throws AlgebricksException {
        return new MarkerSinkOperator();
    }
    ...
    @Override
    public ILogicalOperator visitFixedPointOperator(FixedPointOperator op, Void arg) throws AlgebricksException {
        List<ILogicalPlan> newPlansInLoop = new ArrayList<>();
        Map<LogicalVariable, LogicalVariable> newAnchorMap = new HashMap<>(op.getAnchorOutputMap());
        Map<LogicalVariable, LogicalVariable> newRecursiveMap = new HashMap<>(op.getRecursiveOutputMap());
        FixedPointOperator fixedPointOp = new FixedPointOperator(newAnchorMap, newRecursiveMap, newPlansInLoop);
        fixedPointOp.setOnIsomorphicCopyCallback(op.getOnIsomorphicCopyCallback());
        fixedPointOp.getAnnotations().putAll(op.getAnnotations());
        for (ILogicalPlan plan : op.getNestedPlans()) {
            newPlansInLoop.add(OperatorManipulationUtil.deepCopy(plan, fixedPointOp));
        }
        return fixedPointOp;
    }

    @Override
    public ILogicalOperator visitRecursiveTailOperator(RecursiveTailOperator op, Void arg) throws AlgebricksException {
        // Note: we do not consider the fixed-point reference operator here (similar to NTS).
        return new RecursiveTailOperator(op);
    }

    @Override
    public ILogicalOperator visitRecursiveHeadOperator(RecursiveHeadOperator op, Void arg) throws AlgebricksException {
        Map<LogicalVariable, LogicalVariable> newRecursiveOutputMap = new HashMap<>(op.getRecursiveOutputMap());
        return new RecursiveHeadOperator(newRecursiveOutputMap);
    }
