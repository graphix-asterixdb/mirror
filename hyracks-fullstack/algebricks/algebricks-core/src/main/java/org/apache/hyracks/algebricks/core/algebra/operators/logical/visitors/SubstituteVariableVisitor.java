...
public class SubstituteVariableVisitor
    ...
    @Override
    public Void visitFixedPointOperator(FixedPointOperator op, Pair<LogicalVariable, LogicalVariable> pair)
            throws AlgebricksException {
        Function<Map.Entry<LogicalVariable, LogicalVariable>, Pair<LogicalVariable, LogicalVariable>> f = e -> {
            ...
            if (replacementKey == pair.first) {
                replacementKey = pair.second;
            }
            if (replacementValue == pair.first) {
                replacementValue = pair.second;
            }
            return new Pair<>(replacementKey, replacementValue);
        };
        Map<LogicalVariable, LogicalVariable> replacementAnchorMap = op.getAnchorOutputMap().entrySet().stream().map(f)
                .collect(Collectors.toMap(Pair::getFirst, Pair::getSecond));
        Map<LogicalVariable, LogicalVariable> replacementRecursiveMap = op.getRecursiveOutputMap().entrySet().stream()
                .map(f).collect(Collectors.toMap(Pair::getFirst, Pair::getSecond));
        op.replaceOutputMaps(replacementAnchorMap, replacementRecursiveMap);
        substInNestedPlans(op, pair.first, pair.second);
        return null;
    }

    @Override
    public Void visitRecursiveTailOperator(RecursiveTailOperator op, Pair<LogicalVariable, LogicalVariable> pair)
            throws AlgebricksException {
        Function<Map.Entry<LogicalVariable, LogicalVariable>, Pair<LogicalVariable, LogicalVariable>> f = e -> {
            LogicalVariable replacementKey = e.getKey();
            LogicalVariable replacementValue = e.getValue();
        ...
        Map<LogicalVariable, LogicalVariable> replacementAnchorMap = op.getAnchorInputMap().entrySet().stream().map(f)
                .collect(Collectors.toMap(Pair::getFirst, Pair::getSecond));
        Map<LogicalVariable, LogicalVariable> replacementRecursiveMap = op.getRecursiveInputMap().entrySet().stream()
                .map(f).collect(Collectors.toMap(Pair::getFirst, Pair::getSecond));
        op.replaceInputMaps(replacementAnchorMap, replacementRecursiveMap);
        return null;
    }

    @Override
    public Void visitRecursiveHeadOperator(RecursiveHeadOperator op, Pair<LogicalVariable, LogicalVariable> pair)
            throws AlgebricksException {
        Map<LogicalVariable, LogicalVariable> replacementMap = op.getRecursiveOutputMap().entrySet().stream().map(e -> {
            LogicalVariable replacementKey = e.getKey();
            LogicalVariable replacementValue = e.getValue();
            if (replacementKey == pair.first) {
                replacementKey = pair.second;
        ...
        op.replaceOutputMap(replacementMap);
        return null;
    }
    ...
    @Override
    public Void visitMarkerSinkOperator(MarkerSinkOperator op, Pair<LogicalVariable, LogicalVariable> arg)
            throws AlgebricksException {
        return null;
    }
