...
public class OperatorPropertiesUtil {
    ...
    public static void getOperatorsInLoop(ILogicalOperator inputOp, List<ILogicalOperator> resultList, boolean isBfs) {
        Deque<ILogicalOperator> operatorQueue = new ArrayDeque<>(List.of(inputOp));
        while (!operatorQueue.isEmpty()) {
            ILogicalOperator workingOp = isBfs ? operatorQueue.removeFirst() : operatorQueue.removeLast();
            resultList.add(workingOp);
            switch (workingOp.getOperatorTag()) {
                case FIXED_POINT:
                    FixedPointOperator fpOp = (FixedPointOperator) workingOp;
                    ILogicalPlan planInLoop = fpOp.getNestedPlans().get(0);
                    ILogicalOperator rootOp = planInLoop.getRoots().get(0).getValue();
                    operatorQueue.addLast(rootOp);
                    continue;
            }

            // We are going to exclude our FIXED-POINT ops. For everything else, visit the inputs then the nested plans.
            for (Mutable<ILogicalOperator> inputRef : workingOp.getInputs()) {
                operatorQueue.addLast(inputRef.getValue());
            }
            if (((AbstractLogicalOperator) workingOp).hasNestedPlans()) {
                AbstractOperatorWithNestedPlans nestedPlanOp = (AbstractOperatorWithNestedPlans) workingOp;
                ILogicalPlan nestedPlan = nestedPlanOp.getNestedPlans().get(0);
