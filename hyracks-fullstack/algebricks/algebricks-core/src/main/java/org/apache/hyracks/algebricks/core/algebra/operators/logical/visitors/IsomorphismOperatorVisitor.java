...
public class IsomorphismOperatorVisitor implements ILogicalOperatorVisitor<Boolean, ILogicalOperator> {
    ...
    @Override
    public Boolean visitMarkerSinkOperator(MarkerSinkOperator op, ILogicalOperator arg) throws AlgebricksException {
        AbstractLogicalOperator aop = (AbstractLogicalOperator) copyAndSubstituteVar(op, arg);
        if (aop.getOperatorTag() != LogicalOperatorTag.MARKER_SINK) {
            return Boolean.FALSE;
        }
        return Boolean.TRUE;
    }
    ...
    @Override
    public Boolean visitFixedPointOperator(FixedPointOperator op, ILogicalOperator arg) throws AlgebricksException {
        AbstractLogicalOperator aop = (AbstractLogicalOperator) arg;
        if (aop.getOperatorTag() != LogicalOperatorTag.FIXED_POINT) {
            return Boolean.FALSE;
        }

        // Compare our nested-plans.
        FixedPointOperator fpOpArg = (FixedPointOperator) copyAndSubstituteVar(op, arg);
        List<ILogicalPlan> plans = op.getNestedPlans();
        List<ILogicalPlan> plansArg = fpOpArg.getNestedPlans();
        for (int i = 0; i < plans.size(); i++) {
            List<Mutable<ILogicalOperator>> roots = plans.get(i).getRoots();
            List<Mutable<ILogicalOperator>> rootsArg = plansArg.get(i).getRoots();
            if (roots.size() == rootsArg.size()) {
    ...
    @Override
    public Boolean visitRecursiveTailOperator(RecursiveTailOperator op, ILogicalOperator arg)
            throws AlgebricksException {
        AbstractLogicalOperator aop = (AbstractLogicalOperator) arg;
        if (aop.getOperatorTag() != LogicalOperatorTag.RECURSIVE_TAIL) {
            return Boolean.FALSE;
        }

        // Compare our variable mappings.
        RecursiveTailOperator rtOpArg = (RecursiveTailOperator) copyAndSubstituteVar(op, arg);
        if (!op.getAnchorInputMap().keySet().equals(rtOpArg.getAnchorInputMap().keySet())) {
            return Boolean.FALSE;
        }
        if (!op.getAnchorInputMap().entrySet().equals(rtOpArg.getAnchorInputMap().entrySet())) {
            return Boolean.FALSE;
        }
        if (!op.getRecursiveInputMap().keySet().equals(rtOpArg.getRecursiveInputMap().keySet())) {
            return Boolean.FALSE;
        }
        if (!op.getRecursiveInputMap().entrySet().equals(rtOpArg.getRecursiveInputMap().entrySet())) {
            return Boolean.FALSE;
        }
        return Boolean.TRUE;
    }
    ...
    @Override
    public Boolean visitRecursiveHeadOperator(RecursiveHeadOperator op, ILogicalOperator arg)
            throws AlgebricksException {
        AbstractLogicalOperator aop = (AbstractLogicalOperator) arg;
        if (aop.getOperatorTag() != LogicalOperatorTag.RECURSIVE_HEAD) {
            return Boolean.FALSE;
        }

        // Compare our variable mappings.
        RecursiveHeadOperator rhOpArg = (RecursiveHeadOperator) copyAndSubstituteVar(op, arg);
        if (!op.getRecursiveOutputMap().keySet().equals(rhOpArg.getRecursiveOutputMap().keySet())) {
            return Boolean.FALSE;
        }
        if (!op.getRecursiveOutputMap().entrySet().equals(rhOpArg.getRecursiveOutputMap().entrySet())) {
            return Boolean.FALSE;
        }
        return Boolean.TRUE;
    }
