...
public class OperatorManipulationUtil {
    ...
    public static boolean setOperatorMode(AbstractLogicalOperator op) {
        ...
        switch (op.getOperatorTag()) {
            ...
            case RECURSIVE_TAIL: {
                // We inherit our execution mode from the input to our fixed-point operator.
                RecursiveTailOperator rtOp = (RecursiveTailOperator) op;
                ILogicalOperator fpOp = rtOp.getFixedPointOp();
                ILogicalOperator inputFpOp = fpOp.getInputs().get(0).getValue();
                rtOp.setExecutionMode(inputFpOp.getExecutionMode());
            }
            default: {
