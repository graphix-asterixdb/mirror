...
public class LogicalOperatorPrettyPrintVisitor extends AbstractLogicalOperatorPrettyPrintVisitor<Integer>
    ...
    @Override
    public Void visitFixedPointOperator(FixedPointOperator op, Integer indent) throws AlgebricksException {
        addIndent(indent).append("fixed-point [");
        Iterator<LogicalVariable> it = op.getOutputVariableList().iterator();
        for (int i = 0; it.hasNext(); i++) {
            LogicalVariable outputVariable = it.next();
            if (i > 0) {
                buffer.append(" ");
            }
            buffer.append(outputVariable.toString());
            buffer.append(" <- (");
            buffer.append(op.getInvertedAnchorOutputMap().get(outputVariable).toString());
            buffer.append(",");
            buffer.append(op.getInvertedRecursiveOutputMap().get(outputVariable).toString());
            buffer.append(")");
        }
        buffer.append("] {");
        printNestedPlans(op, indent);
    ...
    @Override
    public Void visitRecursiveTailOperator(RecursiveTailOperator op, Integer indent) throws AlgebricksException {
        addIndent(indent).append("recursive-tail [");
        Iterator<Map.Entry<LogicalVariable, LogicalVariable>> it = op.getAnchorInputMap().entrySet().iterator();
        for (int i = 0; it.hasNext(); i++) {
            Map.Entry<LogicalVariable, LogicalVariable> anchorEntry = it.next();
            if (i > 0) {
                buffer.append(", ");
            }
            buffer.append(anchorEntry.getValue().toString());
            buffer.append(" <- (");
            buffer.append(anchorEntry.getKey().toString());
            buffer.append(",");
            buffer.append(op.getInvertedRecursiveInputMap().get(anchorEntry.getValue()).toString());
            buffer.append(")");
        }
        buffer.append("]");
        return null;
    ...
    @Override
    public Void visitRecursiveHeadOperator(RecursiveHeadOperator op, Integer indent) throws AlgebricksException {
        addIndent(indent).append("recursive-head [");
        Iterator<Map.Entry<LogicalVariable, LogicalVariable>> it = op.getRecursiveOutputMap().entrySet().iterator();
        for (int i = 0; it.hasNext(); i++) {
            Map.Entry<LogicalVariable, LogicalVariable> recursiveEntry = it.next();
            if (i > 0) {
                buffer.append(",");
            }
            buffer.append(recursiveEntry.getValue().toString());
            buffer.append(" <- ");
            buffer.append(recursiveEntry.getKey().toString());
        }
        buffer.append("]");
        return null;
    }
    ...
    @Override
    public Void visitMarkerSinkOperator(MarkerSinkOperator op, Integer indent) throws AlgebricksException {
        addIndent(indent).append("marker-sink");
        return null;
    }
