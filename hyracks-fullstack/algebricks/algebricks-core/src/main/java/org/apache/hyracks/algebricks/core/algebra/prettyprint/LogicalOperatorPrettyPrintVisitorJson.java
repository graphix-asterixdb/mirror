...
public class LogicalOperatorPrettyPrintVisitorJson extends AbstractLogicalOperatorPrettyPrintVisitor<Void>
    ...
    @Override
    public Void visitFixedPointOperator(FixedPointOperator op, Void indent) throws AlgebricksException {
        try {
            jsonGenerator.writeStringField(OPERATOR_FIELD, "fixed-point");
            writeNestedPlans(op, indent);
            return null;
    ...
    public Void visitRecursiveTailOperator(RecursiveTailOperator op, Void arg) throws AlgebricksException {
        try {
            jsonGenerator.writeStringField(OPERATOR_FIELD, "recursive-tail");
            writeArrayFieldOfVariables("from-anchor", new ArrayList<>(op.getAnchorInputMap().keySet()));
            writeArrayFieldOfVariables("from-recursive", new ArrayList<>(op.getRecursiveInputMap().keySet()));
            writeArrayFieldOfVariables("to", new ArrayList<>(op.getPreviousIterationVariables()));
            return null;
    ...
    public Void visitRecursiveHeadOperator(RecursiveHeadOperator op, Void arg) throws AlgebricksException {
        try {
            jsonGenerator.writeStringField(OPERATOR_FIELD, "recursive-head");
            writeArrayFieldOfVariables("from", new ArrayList<>(op.getRecursiveOutputMap().keySet()));
            writeArrayFieldOfVariables("to", new ArrayList<>(op.getRecursiveOutputMap().values()));
            return null;
    ...
    public Void visitMarkerSinkOperator(MarkerSinkOperator op, Void arg) throws AlgebricksException {
        try {
            jsonGenerator.writeStringField(OPERATOR_FIELD, "marker-sink");
            return null;
        } catch (IOException e) {
            throw AlgebricksException.create(ErrorCode.ERROR_PRINTING_PLAN, e, String.valueOf(e));
        }
