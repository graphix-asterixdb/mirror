...
public class JobBuilder implements IHyracksJobBuilder {
    ...
    private void setAllPartitionConstraints(Map<IConnectorDescriptor, TargetConstraint> tgtConstraints)
        ...
        for (OperatorDescriptorId rootId : roots) {
            setPartitionConstraintsBottomup(rootId, tgtConstraints, null, new HashSet<>(), false);
        }
        for (OperatorDescriptorId rootId : roots) {
            setPartitionConstraintsTopdown(rootId, tgtConstraints, null, new HashSet<>());
        }
        for (OperatorDescriptorId rootId : roots) {
            setPartitionConstraintsBottomup(rootId, tgtConstraints, null, new HashSet<>(), true);
        }
    }
    ...
    private void setPartitionConstraintsTopdown(OperatorDescriptorId opId,
            ...
            Set<OperatorDescriptorId> visitedOps) {
        if (visitedOps.contains(opId)) {
            return;
        }
        visitedOps.add(opId);
        ...
        if (opInputs != null) {
            for (IConnectorDescriptor conn : opInputs) {
                ...
                // Post Order DFS
                setPartitionConstraintsTopdown(src.getOperatorId(), tgtConstraints, opDesc, visitedOps);
            }
        }
    }
    ...
    private void setPartitionConstraintsBottomup(OperatorDescriptorId opId,
            Map<IConnectorDescriptor, TargetConstraint> tgtConstraints, IOperatorDescriptor parentOp,
            Set<OperatorDescriptorId> visitedOps, boolean finalPass) throws AlgebricksException {
        if (visitedOps.contains(opId)) {
            return;
        }
        visitedOps.add(opId);
        ...
        if (opInputs != null) {
            for (IConnectorDescriptor conn : opInputs) {
                ...
                IOperatorDescriptor src = p.getLeft().getLeft();
                // Pre-order DFS
                setPartitionConstraintsBottomup(src.getOperatorId(), tgtConstraints, opDesc, visitedOps, finalPass);
                TargetConstraint constraint = tgtConstraints.get(conn);
                if (constraint != null) {
                    switch (constraint) {
                        case ONE:
