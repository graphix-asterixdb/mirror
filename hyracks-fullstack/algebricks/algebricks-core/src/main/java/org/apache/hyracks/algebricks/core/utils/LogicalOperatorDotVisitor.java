...
public class LogicalOperatorDotVisitor implements ILogicalOperatorVisitor<String, Boolean> {
    ...
    @Override
    public String visitFixedPointOperator(FixedPointOperator op, Boolean showDetails) throws AlgebricksException {
        stringBuilder.setLength(0);
        stringBuilder.append("fixed-point {}");
        appendSchema(op, showDetails);
        appendAnnotations(op, showDetails);
        appendPhysicalOperatorInfo(op, showDetails);
        return stringBuilder.toString();
    ...
    @Override
    public String visitRecursiveTailOperator(RecursiveTailOperator op, Boolean showDetails) {
        stringBuilder.setLength(0);
        stringBuilder.append("recursive-tail ");
        stringBuilder.append("(anchor-mapping: ");
        Iterator<Map.Entry<LogicalVariable, LogicalVariable>> anchorIt = op.getAnchorInputMap().entrySet().iterator();
        if (anchorIt.hasNext()) {
            Map.Entry<LogicalVariable, LogicalVariable> anchorEntry = anchorIt.next();
            stringBuilder.append(anchorEntry.getKey().toString());
            stringBuilder.append(" -> ").append(anchorEntry.getValue().toString());
        }
        while (anchorIt.hasNext()) {
            Map.Entry<LogicalVariable, LogicalVariable> anchorEntry = anchorIt.next();
            stringBuilder.append(", ").append(anchorEntry.getKey().toString());
            stringBuilder.append(" -> ").append(anchorEntry.getValue().toString());
        }
        stringBuilder.append(") (recursive-mapping: ");
        Iterator<Map.Entry<LogicalVariable, LogicalVariable>> recursiveIt =
                op.getRecursiveInputMap().entrySet().iterator();
        if (recursiveIt.hasNext()) {
            Map.Entry<LogicalVariable, LogicalVariable> recursiveEntry = recursiveIt.next();
            stringBuilder.append(recursiveEntry.getKey().toString());
            stringBuilder.append(" -> ").append(recursiveEntry.getValue().toString());
        }
        while (recursiveIt.hasNext()) {
            Map.Entry<LogicalVariable, LogicalVariable> recursiveEntry = recursiveIt.next();
            stringBuilder.append(", ").append(recursiveEntry.getKey().toString());
            stringBuilder.append(" -> ").append(recursiveEntry.getValue().toString());
        }
        stringBuilder.append(")");
        return stringBuilder.toString();
    ...
    @Override
    public String visitRecursiveHeadOperator(RecursiveHeadOperator op, Boolean arg) {
        stringBuilder.setLength(0);
        stringBuilder.append("recursive-head ");
        stringBuilder.append("(recursive-mapping: ");
        Iterator<Map.Entry<LogicalVariable, LogicalVariable>> recursiveIt =
                op.getRecursiveOutputMap().entrySet().iterator();
        if (recursiveIt.hasNext()) {
            Map.Entry<LogicalVariable, LogicalVariable> recursiveEntry = recursiveIt.next();
            stringBuilder.append(recursiveEntry.getKey().toString());
            stringBuilder.append(" -> ").append(recursiveEntry.getValue().toString());
    ...
    @Override
    public String visitMarkerSinkOperator(MarkerSinkOperator op, Boolean showDetails) {
        stringBuilder.setLength(0);
        stringBuilder.append("marker sink");
        appendSchema(op, showDetails);
        appendAnnotations(op, showDetails);
        appendPhysicalOperatorInfo(op, showDetails);
        return stringBuilder.toString();
