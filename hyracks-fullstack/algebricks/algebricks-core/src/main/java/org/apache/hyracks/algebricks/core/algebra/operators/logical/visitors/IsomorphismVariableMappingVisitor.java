...
public class IsomorphismVariableMappingVisitor implements ILogicalOperatorVisitor<Void, ILogicalOperator> {
    ...
    @Override
    public Void visitMarkerSinkOperator(MarkerSinkOperator op, ILogicalOperator arg) throws AlgebricksException {
        mapVariablesStandard(op, arg);
        return null;
    }
    ...
    @Override
    public Void visitFixedPointOperator(FixedPointOperator op, ILogicalOperator arg) throws AlgebricksException {
        if (op.getOperatorTag() != arg.getOperatorTag()) {
            return null;
        }
        mapVariables(op, arg);
    ...
    @Override
    public Void visitRecursiveTailOperator(RecursiveTailOperator op, ILogicalOperator arg) throws AlgebricksException {
        if (op.getOperatorTag() != arg.getOperatorTag()) {
            return null;
        }
        mapVariables(op, arg);
        ...
        mappedOps.add(arg);
        RecursiveTailOperator rtOp = (RecursiveTailOperator) arg;
        ILogicalOperator inputToCreator1 = op.getFixedPointOp();
        ILogicalOperator inputToCreator2 = rtOp.getFixedPointOp();
        inputToCreator1.accept(this, inputToCreator2);
        return null;
    }

    @Override
    public Void visitRecursiveHeadOperator(RecursiveHeadOperator op, ILogicalOperator arg) throws AlgebricksException {
        mapVariablesStandard(op, arg);
        return null;
    }
