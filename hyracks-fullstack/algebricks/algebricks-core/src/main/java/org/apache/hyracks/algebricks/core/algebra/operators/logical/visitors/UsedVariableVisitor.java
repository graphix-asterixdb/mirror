...
public class UsedVariableVisitor implements ILogicalOperatorVisitor<Void, Void> {
    ...
    public Void visitExchangeOperator(ExchangeOperator op, Void arg) throws AlgebricksException {
        ...
        if (op.getPhysicalOperator() != null) {
            ...
            if (physOp.getOperatorTag() == PhysicalOperatorTag.STATE_RELEASE) {
                StateReleasePOperator srPOp = (StateReleasePOperator) physOp;
                physOp = srPOp.getPhysicalDelegateOp();
            }
    ...
    public Void visitFixedPointOperator(FixedPointOperator op, Void arg) throws AlgebricksException {
        usedVariables.addAll(op.getAnchorOutputMap().keySet());
        usedVariables.addAll(op.getRecursiveOutputMap().keySet());

        // We also need to grab the anchor input variables from our recursive tail...
        Deque<ILogicalOperator> operatorStack = new ArrayDeque<>();
        ILogicalPlan nestedPlan = op.getNestedPlans().get(0);
        ILogicalOperator rootOp = nestedPlan.getRoots().get(0).getValue();
        operatorStack.addLast(rootOp);
        while (!operatorStack.isEmpty()) {
            ILogicalOperator workingOp = operatorStack.removeLast();
            if (workingOp.getOperatorTag() == LogicalOperatorTag.RECURSIVE_TAIL) {
                RecursiveTailOperator rtOp = (RecursiveTailOperator) workingOp;
                usedVariables.addAll(rtOp.getAnchorInputMap().keySet());
                break;
        ...
        // ...as well as any "free" variables in our recursive branch.
        AbstractLogicalOperator logicalOp = (AbstractLogicalOperator) rootOp;
        Set<LogicalVariable> recursiveBranchFreeVars = new HashSet<>();
        OperatorPropertiesUtil.getFreeVariablesInSelfOrDesc(logicalOp, recursiveBranchFreeVars);
        usedVariables.addAll(recursiveBranchFreeVars);
        return null;
    }

    @Override
    public Void visitRecursiveTailOperator(RecursiveTailOperator op, Void arg) throws AlgebricksException {
        // No variables are used here.
        return null;
    }

    @Override
    public Void visitRecursiveHeadOperator(RecursiveHeadOperator op, Void arg) throws AlgebricksException {
        Set<LogicalVariable> headUsedVariables = new HashSet<>(op.getRecursiveOutputMap().keySet());

        // Walk down to our fixed point. Gather all used variables in between our head and fixed-point.
        Deque<ILogicalOperator> operatorStack = new ArrayDeque<>();
        operatorStack.addLast(op.getInputs().get(0).getValue());
        while (!operatorStack.isEmpty()) {
            ILogicalOperator workingOp = operatorStack.getLast();
            if (workingOp.getOperatorTag() == LogicalOperatorTag.FIXED_POINT) {
                break;
            }
            VariableUtilities.getUsedVariables(workingOp, headUsedVariables);
            for (Mutable<ILogicalOperator> inputRef : workingOp.getInputs()) {
                operatorStack.addLast(inputRef.getValue());
            }
        }
        usedVariables.addAll(headUsedVariables);
        return null;
    }
    ...
    @Override
    public Void visitMarkerSinkOperator(MarkerSinkOperator op, Void arg) throws AlgebricksException {
        return null;
    }
