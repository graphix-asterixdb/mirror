...
public class AlgebricksOptimizationContext implements IOptimizationContext {
    ...
    private final Map<ILogicalOperator, Byte> markerDesignationMap = new HashMap<>();
    ...
    @Override
    public byte getDesignationByteFor(ILogicalOperator representativeOp) {
        if (!markerDesignationMap.containsKey(representativeOp)) {
            Byte newDesignation = markerDesignationMap.values().stream().max(Byte::compareTo).orElse((byte) 1);
            markerDesignationMap.put(representativeOp, newDesignation);
        }
        return markerDesignationMap.get(representativeOp);
    }
