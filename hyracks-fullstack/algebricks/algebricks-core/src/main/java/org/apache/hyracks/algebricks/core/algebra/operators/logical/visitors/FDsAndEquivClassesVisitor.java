...
public class FDsAndEquivClassesVisitor implements ILogicalOperatorVisitor<Void, IOptimizationContext> {
    ...
    public Void visitAssignOperator(AssignOperator op, IOptimizationContext ctx) throws AlgebricksException {
        ...
        // Generate FDs on a per-expression-basis.
        List<FunctionalDependency> outputFDs = new ArrayList<>(getOrComputeFDs(inp1, ctx));
        Iterator<Mutable<ILogicalExpression>> exprRefIterator = op.getExpressions().iterator();
        Iterator<LogicalVariable> varIterator = op.getVariables().iterator();
        while (exprRefIterator.hasNext() && varIterator.hasNext()) {
            Mutable<ILogicalExpression> workingExprRef = exprRefIterator.next();
            LogicalVariable workingVar = varIterator.next();

            // Add to our functional dependencies list.
            List<LogicalVariable> newTailList = new ArrayList<>();
            newTailList.add(workingVar);
            workingExprRef.getValue().getUsedVariables(newTailList);
            outputFDs.add(new FunctionalDependency(List.of(workingVar), newTailList));
        }
        ctx.putFDList(op, outputFDs);
        return null;
    }
    ...
    @Override
    public Void visitMarkerSinkOperator(MarkerSinkOperator op, IOptimizationContext ctx) throws AlgebricksException {
        propagateFDsAndEquivClasses(op, ctx);
        return null;
    }
    ...
    @Override
    public Void visitFixedPointOperator(FixedPointOperator op, IOptimizationContext ctx) throws AlgebricksException {
        Map<LogicalVariable, EquivalenceClass> equivalenceClasses = new HashMap<>();
        ctx.putEquivalenceClassMap(op, equivalenceClasses);

        // Add functional dependencies from our outputs as well.
        List<FunctionalDependency> functionalDependencies = new ArrayList<>();
        ctx.putFDList(op, functionalDependencies);
        for (LogicalVariable outputVar : op.getOutputVariableList()) {
            List<LogicalVariable> headList = new ArrayList<>(List.of(outputVar));
            List<LogicalVariable> tailList = new ArrayList<>(List.of(outputVar));
            functionalDependencies.add(new FunctionalDependency(headList, tailList));
        }

        // Propagate our FDs and equiv-classes.
        ILogicalOperator anchorOp = op.getInputs().get(0).getValue();
        equivalenceClasses.putAll(getOrComputeEqClasses(anchorOp, ctx));
        functionalDependencies.addAll(getOrComputeFDs(anchorOp, ctx));
        for (ILogicalPlan p : op.getNestedPlans()) {
            for (Mutable<ILogicalOperator> rootOpRef : p.getRoots()) {
                ILogicalOperator rootOp = rootOpRef.getValue();
                equivalenceClasses.putAll(getOrComputeEqClasses(rootOp, ctx));
                functionalDependencies.addAll(getOrComputeFDs(rootOp, ctx));
            }
        }
        return null;
    }
    ...
    @Override
    public Void visitRecursiveTailOperator(RecursiveTailOperator op, IOptimizationContext ctx)
            throws AlgebricksException {
        List<FunctionalDependency> functionalDependencies = new ArrayList<>();
        ctx.putEquivalenceClassMap(op, new HashMap<>());
        ctx.putFDList(op, functionalDependencies);

        // We will carry the dependencies from our fixed-point input.
        ILogicalOperator fpOpInput = op.getFixedPointOp().getInputs().get(0).getValue();
        List<FunctionalDependency> fpInputOpDependencies = new ArrayList<>(getOrComputeFDs(fpOpInput, ctx));
        Set<FunctionalDependency> usedDependencies = new HashSet<>();
        for (Map.Entry<LogicalVariable, LogicalVariable> entry : op.getAnchorInputMap().entrySet()) {
            fpInputOpDependencies.stream().filter(fd -> fd.getTail().contains(entry.getKey())).forEach(fd -> {
                List<LogicalVariable> headList = new ArrayList<>(fd.getHead());
                List<LogicalVariable> tailList = new ArrayList<>(fd.getTail());
                functionalDependencies.add(new FunctionalDependency(headList, tailList));
                tailList.addAll(List.of(entry.getKey(), entry.getValue()));
                usedDependencies.add(fd);
            });
        }
        fpInputOpDependencies.removeAll(usedDependencies);
        functionalDependencies.addAll(fpInputOpDependencies);

        // We will also define our recursive dependencies here.
        for (Map.Entry<LogicalVariable, LogicalVariable> entry : op.getRecursiveInputMap().entrySet()) {
            List<LogicalVariable> headList = new ArrayList<>(List.of(entry.getValue()));
            List<LogicalVariable> tailList = new ArrayList<>(List.of(entry.getValue(), entry.getKey()));
            functionalDependencies.add(new FunctionalDependency(headList, tailList));
            tailList.add(op.getInvertedAnchorInputMap().get(entry.getValue()));
        }
        return null;
    }

    @Override
    public Void visitRecursiveHeadOperator(RecursiveHeadOperator op, IOptimizationContext ctx)
            throws AlgebricksException {
        // Note: we do not define FDs and equivalence classes back to our tail here.
        propagateFDsAndEquivClasses(op, ctx);
        return null;
    }
