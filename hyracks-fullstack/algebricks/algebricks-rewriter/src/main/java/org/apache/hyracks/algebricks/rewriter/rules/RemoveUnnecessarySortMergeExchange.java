...
public class RemoveUnnecessarySortMergeExchange implements IAlgebraicRewriteRule {
    ...
    public boolean rewritePost(Mutable<ILogicalOperator> opRef, IOptimizationContext context)
        ...
        IPhysicalOperator op1PhysicalOp = op1.getPhysicalOperator();
        if (op1PhysicalOp == null) {
            return false;
        } else if (op1PhysicalOp.getOperatorTag() == PhysicalOperatorTag.STATE_RELEASE) {
            op1PhysicalOp = ((StateReleasePOperator) op1PhysicalOp).getPhysicalDelegateOp();
        }
        if (op1PhysicalOp.getOperatorTag() != PhysicalOperatorTag.HASH_PARTITION_EXCHANGE
                && op1PhysicalOp.getOperatorTag() != PhysicalOperatorTag.HASH_PARTITION_MERGE_EXCHANGE) {
            return false;
        }
        ...
        while (currentOp != null) {
            ...
            if (physicalOp == null) {
                return false;
            } else if (physicalOp.getOperatorTag() == PhysicalOperatorTag.STATE_RELEASE) {
                physicalOp = ((StateReleasePOperator) physicalOp).getPhysicalDelegateOp();
            }
            if (physicalOp.getOperatorTag() == PhysicalOperatorTag.SORT_MERGE_EXCHANGE) {
                break;
        ...
        if (op1PhysicalOp.getOperatorTag() == PhysicalOperatorTag.HASH_PARTITION_MERGE_EXCHANGE) {
        ...
        SortMergeExchangePOperator sme = (SortMergeExchangePOperator) ((currentOp.getPhysicalOperator()
                .getOperatorTag() == PhysicalOperatorTag.STATE_RELEASE)
                        ? ((StateReleasePOperator) currentOp.getPhysicalOperator()).getPhysicalDelegateOp()
                        : currentOp.getPhysicalOperator());
        HashPartitionExchangePOperator hpe = (HashPartitionExchangePOperator) op1PhysicalOp;
