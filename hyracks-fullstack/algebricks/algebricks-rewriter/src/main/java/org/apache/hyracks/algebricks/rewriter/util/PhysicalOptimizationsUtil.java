...
public class PhysicalOptimizationsUtil {
    ...
    private static <R> void computeFDsAndEqClassesWithVisitorRec(ILogicalOperator op, IOptimizationContext ctx,
        ...
        if (op.getOperatorTag() == LogicalOperatorTag.NESTEDTUPLESOURCE) {
            ...
            RecursiveTailOperator rt = (RecursiveTailOperator) op;
            ILogicalOperator anchor = rt.getFixedPointOp().getInputs().get(0).getValue();
            if (!visitSet.contains(anchor)) {
                computeFDsAndEqClassesWithVisitorRec(anchor, ctx, visitor, visitSet);
            }
        }
