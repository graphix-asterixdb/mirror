...
public class EnforceStructuralPropertiesRule implements IAlgebraicRewriteRule {
    ...
    private boolean physOptimizeOp(Mutable<ILogicalOperator> opRef, IPhysicalPropertiesVector required,
        ...
        if (op.hasNestedPlans()) {
            ...
            // FP-generated plans should not be treated as nested-plans here.
            boolean isNestedPlan = op.getOperatorTag() != LogicalOperatorTag.FIXED_POINT;
            for (ILogicalPlan p : nested.getNestedPlans()) {
                if (physOptimizePlan(p, required, isNestedPlan, context)) {
                    changed = true;
                }
            }
        }
