...
public class FrameHelper {
    ...
    public static boolean isMessageFrame(ByteBuffer buffer) {
        int allFrameSize = IntSerDeUtils.getInt(buffer.array(), FrameConstants.META_DATA_FRAME_COUNT_OFFSET);
        if (allFrameSize != 1) {
            return false;
        }
        for (int i = 0; i < FrameConstants.MESSAGE_FRAME_IDENTIFIER_VALUE.length; i++) {
            byte b = FrameConstants.MESSAGE_FRAME_IDENTIFIER_VALUE[i];
            if (buffer.array()[buffer.limit() - FrameConstants.MESSAGE_FRAME_IDENTIFIER_VALUE.length + i] != b) {
                return false;
            }
        }
        return true;
