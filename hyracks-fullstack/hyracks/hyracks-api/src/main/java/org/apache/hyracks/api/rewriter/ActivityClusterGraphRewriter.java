...
public class ActivityClusterGraphRewriter {
    // TODO (GLENN): This class should probably in the dataflow module?
    private static final String ONE_TO_ONE_CONNECTOR =
            "org.apache.hyracks.dataflow.std.connectors." + "OneToOneConnectorDescriptor";

    private static final String[] EXEMPT_OPERATORS =
            new String[] { "org.apache.hyracks.dataflow.std.iteration.FixedPointOperatorDescriptor",
                    "org.apache.hyracks.dataflow.std.iteration.FixedPointBaseOperatorDescriptor", };
            ...
            for (Entry<ActivityId, SuperActivity> entry : clonedSuperActivities.entrySet()) {
                ...
                if (outputConnectors != null) {
                    for (IConnectorDescriptor outputConn : outputConnectors) {
                        ...
                        SuperActivity existingSuperActivity = invertedActivitySuperActivityMap.get(consumerActivity);
                        if (outputConn.getClass().getName().contains(ONE_TO_ONE_CONNECTOR)
                                && Arrays.stream(EXEMPT_OPERATORS).noneMatch(s -> {
                                    boolean isProducerExempt = producerActivity.getClass().getName().contains(s);
                                    boolean isConsumerExempt = consumerActivity.getClass().getName().contains(s);
                                    return isProducerExempt || isConsumerExempt;
                                })) {
                            /*
                             * expend the super activity cluster on an one-to-one out-bound connection
                             */
                            if (existingSuperActivity == null) {
                                superActivity.addActivity(consumerActivity);
                                toBeExpended.add(consumerActivity);
                                invertedActivitySuperActivityMap.put(consumerActivity, superActivity);
                            } else {
                                /*
                            ...
                            if (existingSuperActivity == null) {
                                ...
                                createNewSuperActivity(ac, superActivities, toBeExpendedMap,
                                        invertedActivitySuperActivityMap, consumerActivity.getActivityId(),
                                        consumerActivity);
                            }
                        }
                    }
