...
import org.apache.hyracks.dataflow.std.connectors.MessageAwareConnectorDescriptor;
...
import org.apache.hyracks.tests.util.MarkerLocalInjectorOperatorDescriptor;
...
public class TPCHCustomerOrderHashJoinTest extends AbstractIntegrationTest {
    ...
    public void customerOrderCIDHybridHashJoinMulti() throws Exception {
        ...
        spec.addRoot(printer);
        //        runTest(spec);
        runTestAndStoreResult(spec, new File("target/hhj.txt"));
    }

    @Test
    public void customerOrderCIDPBJoin() throws Exception {
        JobSpecification spec = new JobSpecification();

        // Build our customer scanner.
        FileSplit[] custSplits = new FileSplit[] {
                new ManagedFileSplit(NC1_ID, FileUtil.joinPath("data", "tpch0.001", "customer-part1.tbl")),
                new ManagedFileSplit(NC2_ID, FileUtil.joinPath("data", "tpch0.001", "customer-part2.tbl")) };
        IFileSplitProvider custSplitsProvider = new ConstantFileSplitProvider(custSplits);
        FileScanOperatorDescriptor custScanner = new FileScanOperatorDescriptor(spec, custSplitsProvider,
                new DelimitedDataTupleParserFactory(custValueParserFactories, '|'), custDesc);
        PartitionConstraintHelper.addAbsoluteLocationConstraint(spec, custScanner, NC1_ID, NC2_ID);

        // Build our order scanner.
        FileSplit[] ordersSplits = new FileSplit[] {
                new ManagedFileSplit(NC1_ID, FileUtil.joinPath("data", "tpch0.001", "orders-part1.tbl")),
                new ManagedFileSplit(NC2_ID, FileUtil.joinPath("data", "tpch0.001", "orders-part2.tbl")) };
        IFileSplitProvider ordersSplitsProvider = new ConstantFileSplitProvider(ordersSplits);
        FileScanOperatorDescriptor ordScanner = new FileScanOperatorDescriptor(spec, ordersSplitsProvider,
                new DelimitedDataTupleParserFactory(orderValueParserFactories, '|'), ordersDesc);
        PartitionConstraintHelper.addAbsoluteLocationConstraint(spec, ordScanner, NC1_ID, NC2_ID);

        // Our probe-side is order. Add a marker in between each tuple.
        MarkerLocalInjectorOperatorDescriptor orderMarker =
                new MarkerLocalInjectorOperatorDescriptor(spec, ordersDesc, (byte) 1);
        PartitionConstraintHelper.addAbsoluteLocationConstraint(spec, orderMarker, NC1_ID, NC2_ID);
        IConnectorDescriptor markerScanConn = new OneToOneConnectorDescriptor(spec);
        IConnectorDescriptor messageAwareScanConn = new MessageAwareConnectorDescriptor(spec, markerScanConn);
        spec.connect(messageAwareScanConn, ordScanner, 0, orderMarker, 0);
        spec.getConnectorMap().remove(markerScanConn.getConnectorId());

        // Build our PBJ operator.
        IOperatorDescriptor join = new PersistentBuildJoinOperatorDescriptor(spec, 8, 20, 1.2, new int[] { 1 },
                new int[] { 0 }, new IBinaryHashFunctionFamily[] { MurmurHash3BinaryHashFunctionFamily.INSTANCE },
                new IBinaryHashFunctionFamily[] { MurmurHash3BinaryHashFunctionFamily.INSTANCE }, custOrderJoinDesc,
                new JoinComparatorFactory(UTF8StringBinaryComparatorFactory.INSTANCE, 1, 0),
                new JoinComparatorFactory(UTF8StringBinaryComparatorFactory.INSTANCE, 0, 1), null, null, (byte) 1,
                false, null);
        PartitionConstraintHelper.addAbsoluteLocationConstraint(spec, join, NC1_ID, NC2_ID);
        IConnectorDescriptor ordJoinConn = new MToNPartitioningConnectorDescriptor(spec,
                FieldHashPartitionComputerFactory.of(new int[] { 1 }, new IBinaryHashFunctionFactory[] {
                        PointableBinaryHashFunctionFactory.of(UTF8StringPointable.FACTORY) }));
        IConnectorDescriptor messageAwareJoinConn = new MessageAwareConnectorDescriptor(spec, ordJoinConn);
        spec.connect(messageAwareJoinConn, orderMarker, 0, join, 0);
        spec.getConnectorMap().remove(ordJoinConn.getConnectorId());
        IConnectorDescriptor custJoinConn = new MToNPartitioningConnectorDescriptor(spec,
                FieldHashPartitionComputerFactory.of(new int[] { 0 }, new IBinaryHashFunctionFactory[] {
                        PointableBinaryHashFunctionFactory.of(UTF8StringPointable.FACTORY) }));
        spec.connect(custJoinConn, custScanner, 0, join, 1);

        // All markers going into our result should be discarded.
        IOperatorDescriptor markerSink = new MessageSinkOperatorDescriptor(spec, custOrderJoinDesc);
        PartitionConstraintHelper.addAbsoluteLocationConstraint(spec, markerSink, NC1_ID, NC2_ID);
        IConnectorDescriptor joinMarkerSinkConn = new OneToOneConnectorDescriptor(spec);
        IConnectorDescriptor markerAwareSinkConn = new MessageAwareConnectorDescriptor(spec, joinMarkerSinkConn);
        spec.connect(markerAwareSinkConn, join, 0, markerSink, 0);
        spec.getConnectorMap().remove(joinMarkerSinkConn.getConnectorId());

        // Build our result printer.
        ResultSetId rsId = new ResultSetId(1);
        spec.addResultSetId(rsId);
        AbstractSingleActivityOperatorDescriptor printer = new ResultWriterOperatorDescriptor(spec, rsId, null, false,
                ResultSerializerFactoryProvider.INSTANCE.getResultSerializerFactoryProvider(), 1);
        PartitionConstraintHelper.addAbsoluteLocationConstraint(spec, printer, NC1_ID);
        IConnectorDescriptor markerSinkPrinterConn = new MToNBroadcastConnectorDescriptor(spec);
        spec.connect(markerSinkPrinterConn, markerSink, 0, printer, 0);
        spec.addRoot(printer);

        // Run our test.
        //        runTest(spec);
        runTestAndStoreResult(spec, new File("target/pbj.txt"));
    }
