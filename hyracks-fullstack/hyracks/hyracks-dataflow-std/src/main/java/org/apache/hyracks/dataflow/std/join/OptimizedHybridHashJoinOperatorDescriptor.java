...
public class OptimizedHybridHashJoinOperatorDescriptor extends AbstractOperatorDescriptor {
    ...
    protected static final int BUILD_AND_PARTITION_ACTIVITY_ID = 0;
    protected static final int PARTITION_AND_JOIN_ACTIVITY_ID = 1;
    ...
    protected final IMissingWriterFactory[] nonMatchWriterFactories;

    //Flags added for test purpose
    private boolean skipInMemoryHJ = true;
    private boolean forceNLJ = true;
    ...
    public static class BuildAndPartitionTaskState extends AbstractStateObject {
        ...
        protected int memForJoin;
        protected int numOfPartitions;
        protected OptimizedHybridHashJoin hybridHJ;
    ...
    protected static class PartitionAndJoinEnvironment implements Serializable {
        private static final long serialVersionUID = 1L;

        protected IFrame rPartbuff;
        protected FrameTupleAppender nullResultAppender;
        protected FrameTupleAccessor probeTupleAccessor;
     ...
     */
    protected class PartitionAndBuildActivityNode extends AbstractActivityNode {
        private static final long serialVersionUID = 1L;
        ...
        public IOperatorNodePushable createPushRuntime(final IHyracksTaskContext ctx,
                ...
                throws HyracksDataException {
            return new PartitionAndBuildPushRuntime(ctx, recordDescProvider, partition, nPartitions);
        }

        protected class PartitionAndBuildPushRuntime extends AbstractUnaryInputSinkOperatorNodePushable {
            ...
            private final ITuplePartitionComputer buildHpc =
                    new FieldHashPartitionComputerFamily(buildKeys, buildHashFunctionFactories)
                            .createPartitioner(INIT_SEED);
            private IOperatorStats stats = new NoOpOperatorStats();
            boolean failed = false;

            public PartitionAndBuildPushRuntime(final IHyracksTaskContext ctx,
                    IRecordDescriptorProvider recordDescProvider, final int partition, final int nPartitions) {
                this.buildRd = recordDescProvider.getInputRecordDescriptor(getActivityId(), 0);
                this.probeRd = recordDescProvider.getInputRecordDescriptor(probeAid, 0);
                this.buildPredEval =
                        (buildPredEvalFactory == null ? null : buildPredEvalFactory.createPredicateEvaluator());
                this.probePredEval = (probePredEvalFactory == null || isLeftOuter ? null
                        : probePredEvalFactory.createPredicateEvaluator());

                this.state = new BuildAndPartitionTaskState(ctx.getJobletContext().getJobId(),
                        new TaskId(getActivityId(), partition));
                this.ctx = ctx;
                this.nPartitions = nPartitions;
            }

            protected ISerializableTable createSerializableTable(int tableSize, IHyracksJobletContext jobletCtx,
                    ISimpleFrameBufferManager bufferManager) throws HyracksDataException {
                return new SerializableHashTable(tableSize, jobletCtx, bufferManager);
            }
            ...
            public void open() throws HyracksDataException {
                ...
                state.hybridHJ =
                        new OptimizedHybridHashJoin(ctx.getJobletContext(), state.memForJoin, state.numOfPartitions,
                                PROBE_REL, BUILD_REL, probeRd, buildRd, probeHpc, buildHpc, probePredEval,
                                buildPredEval, isLeftOuter, nonMatchWriterFactories, this::createSerializableTable);
     ...
     */
    protected class ProbeAndJoinActivityNode extends AbstractActivityNode {
        ...
        protected class ProbeAndJoinPushRuntime extends AbstractUnaryInputUnaryOutputOperatorNodePushable {
            protected final IHyracksTaskContext ctx;
            protected final IHyracksJobletContext jobletCtx;
            protected BuildAndPartitionTaskState state;

            protected boolean failed = false;
            protected IOperatorStats stats = new NoOpOperatorStats();

            protected final RecordDescriptor buildRd;
            protected final RecordDescriptor probeRd;
            protected final ITuplePairComparator probComp;
            protected final ITuplePairComparator buildComp;
            protected final int partition;
            protected final int nPartitions;

            public ProbeAndJoinPushRuntime(final IHyracksTaskContext ctx, IRecordDescriptorProvider recordDescProvider,
                    final int partition, final int nPartitions) throws HyracksDataException {
                this.ctx = ctx;
                this.jobletCtx = ctx.getJobletContext();
                this.buildRd = recordDescProvider.getInputRecordDescriptor(buildAid, 0);
                this.probeRd = recordDescProvider.getInputRecordDescriptor(getActivityId(), 0);
                this.probComp = tuplePairComparatorFactoryProbe2Build.createTuplePairComparator(ctx);
                this.buildComp = tuplePairComparatorFactoryBuild2Probe.createTuplePairComparator(ctx);
                this.partition = partition;
                this.nPartitions = nPartitions;
            }

            protected PartitionAndJoinEnvironment createEnvironment() throws HyracksDataException {
                PartitionAndJoinEnvironment environment = new PartitionAndJoinEnvironment();
                environment.rPartbuff = new VSizeFrame(jobletCtx);
                environment.nonMatchWriter = isLeftOuter ? new IMissingWriter[nonMatchWriterFactories.length] : null;
                environment.nullTupleBuild = isLeftOuter ? new ArrayTupleBuilder(buildRd.getFieldCount()) : null;
                if (isLeftOuter) {
                    DataOutput out = environment.nullTupleBuild.getDataOutput();
                    for (int i = 0; i < nonMatchWriterFactories.length; i++) {
                        environment.nonMatchWriter[i] = nonMatchWriterFactories[i].createMissingWriter();
                        environment.nonMatchWriter[i].writeMissing(out);
                        environment.nullTupleBuild.addFieldEndOffset();
                    }
                }
                return environment;
            }
            ...
            public void close() throws HyracksDataException {
                ...
                try {
                    try {
                        ...
                        state.hybridHJ.releaseResources();
                    }
                    BitSet partitionStatus = state.hybridHJ.getPartitionStatus();
                    PartitionAndJoinEnvironment joinEnv = createEnvironment();
                    joinEnv.rPartbuff.reset();
                    for (int pid = partitionStatus.nextSetBit(0); pid >= 0; pid = partitionStatus.nextSetBit(pid + 1)) {
                        ...
                        if (bReader == null || pReader == null) {
                            if (isLeftOuter && pReader != null) {
                                appendNullToProbeTuples(pReader, joinEnv);
                            }
                            if (bReader != null) {
                                bReader.close();
                            }
                        ...
                        joinPartitionPair(bReader, pReader, bSize, pSize, joinEnv, 1);
                    }
                } catch (Exception e) {
                    if (state.hybridHJ != null) {
                        state.hybridHJ.fail();
            ...
            protected void logProbeComplete() {
                if (LOGGER.isDebugEnabled()) {
            ...
            protected void joinPartitionPair(RunFileReader buildSideReader, RunFileReader probeSideReader,
                    int buildSizeInTuple, int probeSizeInTuple, PartitionAndJoinEnvironment joinEnv, int level)
                    throws HyracksDataException {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("\n>>>Joining Partition Pairs (thread_id " + Thread.currentThread().getId() + ") (pid "
                            + ") - (level " + level + ")" + " - BuildSize:\t" + buildPartSize + "\tProbeSize:\t"
                            + probePartSize + " - MemForJoin " + (state.memForJoin) + "  - LeftOuter is "
                            + isLeftOuter);
                }
                ...
                if (!skipInMemoryHJ && ((buildPartSize + expectedHashTableSizeForBuildInFrame < state.memForJoin)
                        || (probePartSize + expectedHashTableSizeForProbeInFrame < state.memForJoin && !isLeftOuter))) {
                    ...
                    if (!forceRoleReversal && (isLeftOuter || (buildPartSize < probePartSize))) {
                        ...
                        applyInMemHashJoin(buildKeys, probeKeys, tabSize, buildRd, probeRd, buildHpc, probeHpc,
                                buildSideReader, probeSideReader, probComp, joinEnv); // checked-confirmed
                        ...
                        applyInMemHashJoin(probeKeys, buildKeys, tabSize, probeRd, buildRd, probeHpc, buildHpc,
                                probeSideReader, buildSideReader, buildComp, joinEnv); // checked-confirmed
                    }
                }
                //Apply (Recursive) HHJ
                else {
                    ...
                    if (!forceRoleReversal && (isLeftOuter || buildPartSize < probePartSize)) {
                        ...
                        if (LOGGER.isDebugEnabled()) {
                            LOGGER.debug("\t\t>>>Case 2.1 - RecursiveHHJ WITH (isLeftOuter || build<probe) - [Level "
                                    + level + "]");
                        }
                        applyHybridHashJoin((int) buildPartSize, PROBE_REL, BUILD_REL, probeKeys, buildKeys, probeRd,
                                buildRd, probeHpc, buildHpc, probeSideReader, buildSideReader, level, beforeMax,
                                probComp, joinEnv);
                        ...
                        applyHybridHashJoin((int) probePartSize, BUILD_REL, PROBE_REL, buildKeys, probeKeys, buildRd,
                                probeRd, buildHpc, probeHpc, buildSideReader, probeSideReader, level, beforeMax,
                                buildComp, joinEnv);
            ...
            protected void applyHybridHashJoin(int tableSize, final String PROBE_REL, final String BUILD_REL,
                    ...
                    PartitionAndJoinEnvironment joinEnv) throws HyracksDataException {
                ...
                rHHj = new OptimizedHybridHashJoin(jobletCtx, state.memForJoin, n, PROBE_REL, BUILD_REL, probeRd,
                        buildRd, probeHpc, buildHpc, null, null, isLeftOuter, nonMatchWriterFactories,
                        SerializableHashTable::new); //checked-confirmed
                ...
                try {
                    ...
                    try {
                        rHHj.initBuild();
                        joinEnv.rPartbuff.reset();
                        while (buildSideReader.nextFrame(joinEnv.rPartbuff)) {
                            rHHj.build(joinEnv.rPartbuff.getBuffer());
                        }
                    } finally {
                ...
                try {
                    ...
                    try {
                        rHHj.initProbe(comp);
                        while (probeSideReader.nextFrame(joinEnv.rPartbuff)) {
                            rHHj.probe(joinEnv.rPartbuff.getBuffer(), writer);
                        }
                        rHHj.completeProbe(writer);
                    } finally {
                        rHHj.releaseResources();
                    }
                } finally {
                    // Makes sure that files are always properly closed.
                    ...
                    if (!forceNLJ && (afterMax < (NLJ_SWITCH_THRESHOLD * beforeMax))) {
                        ...
                        for (int rPid = rPStatus.nextSetBit(0); rPid >= 0; rPid = rPStatus.nextSetBit(rPid + 1)) {
                            ...
                            if (rbrfw == null || rprfw == null) {
                                if (isLeftOuter && rprfw != null) {
                                    ...
                                    appendNullToProbeTuples(rprfw, joinEnv);
                                }
                                if (rbrfw != null) {
                                    rbrfw.close();
                                }
                            ...
                            if (isReversed) {
                                joinPartitionPair(rprfw, rbrfw, rpSizeInTuple, rbSizeInTuple, joinEnv, level + 1);
                            } else {
                                joinPartitionPair(rbrfw, rprfw, rbSizeInTuple, rpSizeInTuple, joinEnv, level + 1);
                            }
                        }
                        ...
                        for (int rPid = rPStatus.nextSetBit(0); rPid >= 0; rPid = rPStatus.nextSetBit(rPid + 1)) {
                            ...
                            if (isLeftOuter || probeSideInTups < buildSideInTups) {
                                //checked-modified
                                applyNestedLoopJoin(probeRd, buildRd, memSizeInFrames, rprfw, rbrfw, joinEnv);
                            } else {
                                //checked-modified
                                applyNestedLoopJoin(buildRd, probeRd, memSizeInFrames, rbrfw, rprfw, joinEnv);
                            }
                        }
                    }
                } catch (Exception e) {
            ...
            protected void appendNullToProbeTuples(RunFileReader probReader, PartitionAndJoinEnvironment joinEnv)
                    throws HyracksDataException {
                if (joinEnv.nullResultAppender == null) {
                    joinEnv.nullResultAppender = new FrameTupleAppender(new VSizeFrame(jobletCtx));
                }
                if (joinEnv.probeTupleAccessor == null) {
                    joinEnv.probeTupleAccessor = new FrameTupleAccessor(probeRd);
                }
                try {
                    probReader.open();
                    while (probReader.nextFrame(joinEnv.rPartbuff)) {
                        joinEnv.probeTupleAccessor.reset(joinEnv.rPartbuff.getBuffer());
                        for (int tid = 0; tid < joinEnv.probeTupleAccessor.getTupleCount(); tid++) {
                            FrameUtils.appendConcatToWriter(writer, joinEnv.nullResultAppender,
                                    joinEnv.probeTupleAccessor, tid, joinEnv.nullTupleBuild.getFieldEndOffsets(),
                                    joinEnv.nullTupleBuild.getByteArray(), 0, joinEnv.nullTupleBuild.getSize());
                        }
                    }
                    joinEnv.nullResultAppender.write(writer, true);
                } finally {
                    probReader.close();
                }
            }

            protected void applyInMemHashJoin(int[] bKeys, int[] pKeys, int tabSize, RecordDescriptor buildRDesc,
                    ...
                    ITuplePairComparator comp, PartitionAndJoinEnvironment joinEnv) throws HyracksDataException {
                ...
                InMemoryHashJoin joiner = new InMemoryHashJoin(jobletCtx, new FrameTupleAccessor(probeRDesc),
                        ...
                        joinEnv.nonMatchWriter, table, isReversed, bufferManager);
                joiner.setComparator(comp);
                try {
                    bReader.open();
                    joinEnv.rPartbuff.reset();
                    while (bReader.nextFrame(joinEnv.rPartbuff)) {
                        stats.getBytesRead().update(joinEnv.rPartbuff.getBuffer().limit());
                        // We need to allocate a copyBuffer, because this buffer gets added to the buffers list
                        // in the InMemoryHashJoin.
                        ByteBuffer copyBuffer = bufferManager.acquireFrame(joinEnv.rPartbuff.getFrameSize());
                        ...
                        if (copyBuffer == null) {
                            if (joiner.compactHashTable() > 0) {
                                copyBuffer = bufferManager.acquireFrame(joinEnv.rPartbuff.getFrameSize());
                            }
                            if (copyBuffer == null) {
                        ...
                        FrameUtils.copyAndFlip(joinEnv.rPartbuff.getBuffer(), copyBuffer);
                ...
                try {
                    ...
                    joinEnv.rPartbuff.reset();
                    try {
                        while (pReader.nextFrame(joinEnv.rPartbuff)) {
                            if (stats != null) {
                                stats.getBytesRead().update(joinEnv.rPartbuff.getBuffer().limit());
                            }
                            joiner.join(joinEnv.rPartbuff.getBuffer(), writer);
                            joinEnv.rPartbuff.reset();
                        }
            ...
            protected void applyNestedLoopJoin(RecordDescriptor outerRd, RecordDescriptor innerRd, int memorySize,
                    RunFileReader outerReader, RunFileReader innerReader, PartitionAndJoinEnvironment joinEnv)
                    throws HyracksDataException {
                ...
                NestedLoopJoin nlj = new NestedLoopJoin(jobletCtx, new FrameTupleAccessor(outerRd),
                        new FrameTupleAccessor(innerRd), memorySize, isLeftOuter, joinEnv.nonMatchWriter, isReversed);
