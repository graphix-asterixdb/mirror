...
import org.apache.hyracks.dataflow.std.structures.ISerializableTableFactory;
...
public class OptimizedHybridHashJoin {
    ...
    protected FrameTupleAppender bigFrameAppender;
    ...
    protected final IPredicateEvaluator buildPredEval;
    protected final IPredicateEvaluator probePredEval;
    protected final boolean isLeftOuter;
    protected final IMissingWriterFactory[] nullWriterFactories;
    ...
    // To reuse our in-memory-joiner, we also provide an option to write our hash table to disk.
    protected final ISerializableTableFactory tableFactory;
    protected ISerializableTable table;
    ...
    public OptimizedHybridHashJoin(IHyracksJobletContext jobletCtx, int memSizeInFrames, int numOfPartitions,
            ...
            IPredicateEvaluator buildPredEval, boolean isLeftOuter, IMissingWriterFactory[] nullWriterFactories,
            ISerializableTableFactory tableFactory) {
        ...
        this.nonMatchWriters = isLeftOuter ? new IMissingWriter[nullWriterFactories.length] : null;
        if (isLeftOuter) {
            for (int i = 0; i < nullWriterFactories.length; i++) {
                nonMatchWriters[i] = nullWriterFactories[i].createMissingWriter();
            }
        }
    ...
    public void closeBuild() throws HyracksDataException {
        ...
        this.table = tableFactory.getSerializableTable(inMemTupCount, jobletCtx, bufferManagerForHashTable);
     ...
     */
    private int selectAPartitionToReload(long freeSpace, int pid, int inMemTupCount) throws HyracksDataException {
        int frameSize = jobletCtx.getInitialFrameSize();
